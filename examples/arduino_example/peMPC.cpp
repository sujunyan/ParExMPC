...
/* 
    ...
    if trans == 't': y = a*A'*x
    A \in R^{M \times N}
    In case the blas is not available, use self-defined instead, 
...
void solve_couple(const double *xi, const double *yin, const double *x0,    
    ...
    double *pn, *ximyn, *ln, *Ric_Lam_invn, *yn;  
    double const *Ric_Ln , *Ric_Pn; 
    double *del_lamn;
...
/* mpt evaluation function using sequential search */
...
const double *HTB, const double *GTB, const double *FTB){
   ...
   /* initialize values of the tie-break function */
    ...
    for (ireg0=0; ireg0<mpt_nr; ireg0++) {
        ...
        for (ic=0; ic<nc; ic++) {
            hx = 0;
            // tailored for arduino
            //memcpy_P(A_buffer,&A[abspos*domain+ic*domain],domain*sizeof(double));
            
            for (ix=0; ix<domain; ix++) {
                hx += A[abspos*domain+ic*domain+ix]*X[ix];
                //Serial.println(F("Test"));
                //Serial.println(Aval);
                //Aval = pgm_read_float(&A[abspos*domain+ic*domain+ix]);
                //Serial.println(Aval);
                //hx += A_buffer[ix]*X[ix];
                //Serial.println(A_buffer[0]);
            }
            //Bval = pgm_read_float(&B[abspos+ic]);
            if ((hx - B[abspos+ic]) > MPT_ABSTOL) {
            //if ((hx - Bval) > MPT_ABSTOL) {
