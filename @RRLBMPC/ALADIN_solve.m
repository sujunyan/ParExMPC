 ...
 function [z1_next, z2_next, lam_next, u0] = ALADIN_solve(obj, z1, z2, lam, maxiter)
    ...
    for iter = 1:maxiter
        [z1_next, z2_next, xi1, xi2 lam_next, u0] = ALADIN_one_iteration(obj, z1, z2, lam);
        z1 = z1_next;
        z2 = z2_next;
        lam = lam_next;
        % fprintf(" ALADIN iteration %d completed. z1 error %.2f z2 error %.2f lam_diff %.2f \n", iter, norm(z1-xi1), norm(z2 - xi2), norm(lam - lam_next));
    end
    % z1_next = xi1;
    % z2_next = xi2;
...
function [z1_next, z2_next, xi1, xi2, lam_next, u0] = ALADIN_one_iteration(obj, z1, z2, lam)
    ...
    [z, lam_next] = solveQPeq(obj, H, g, Aeq, beq);
...
function [x, lam] = solveQPeq(obj, Q, q, A, b)
    ...
    KKT_mat = [Q, A'; A, zeros(size(A,1), size(A,1))];
    KKT_res = [-q; b];
    x_lam = KKT_mat\KKT_res;

    x = x_lam(1:size(Q,1));
    lam = x_lam(size(Q,1)+1:end);
